...
big_integer::big_integer(bool sign, const ui_vector &data) : sign(sign), data(data) {
    fit();
}
...
void mul(ui_vector const& a, ui_vector const& b, ui_vector& res) {
...
void mul_long_short(ui_vector const& a, ui b, ui_vector& res) {
    size_t n = a.size();
    res.resize(n + 1);
    ull carry = 0;
    for (size_t i = 0; i < n; ++i) {
...
void sub_vectors(ui_vector &a, ui_vector const &b) {
...
bool compare_vectors(ui_vector const &a, ui_vector const &b) {
